...
def run_blcrypt(command, input_file, output_file, steam_id):
    """Run the blcrypt.py command"""
    script_dir = Path(__file__).parent
...
def main():
    ...
    if extension == '.yaml':
        ...
        if run_blcrypt('encrypt', input_file, output_file, steam_id):
        ...
        if run_blcrypt('decrypt', input_file, output_file, steam_id):
            show_popup("Success", f"Successfully written to {output_file.name}", 'info')
